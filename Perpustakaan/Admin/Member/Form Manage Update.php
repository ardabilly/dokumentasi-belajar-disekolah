...
if(isset($_GET["id"]) && strlen(trim($_GET["id"])) != 0 && !$c_Member->bool_Check_MemberID(trim($_GET["id"])))
{
    echo $c_Variable->string_Set_MessageBox("This member data, no found. Can't continue update");
    echo $c_Variable->string_Set_RedirectPage("employee.php");
}
...
if(isset($_POST["btn_Submit"]))
{
    ...
    if($b_Validation)
    {
        $b_Check = $c_Member->bool_Update_MemberData($s_MemberID, $s_FullName, $s_Gender, $s_Religion, $s_PlaceOfBirth, $s_DateOfBirth, $s_PhoneNumber, $s_Email, $s_Address);
        if($b_Check)
        {
...
if(!isset($_POST["btn_Submit"]))
{
    $array_DataMember = $c_Member->array_Get_DataMember($_GET["id"]);

    $s_FullName = $array_DataMember[1];
    $s_Gender = $array_DataMember[2];
    $s_Religion = $array_DataMember[3];
    $s_PlaceOfBirth = $array_DataMember[4];
    $s_DateOfBirth = explode(" ", $array_DataMember[5])[0];
    $s_PhoneNumber = $array_DataMember[6];
    $s_Email = $array_DataMember[7];
    $s_Address = $array_DataMember[8];
}
?>
...
<div class="Row">
<span class="TitleMessage">Form Edit Data Member</span>
</div>
<div class="Row">
